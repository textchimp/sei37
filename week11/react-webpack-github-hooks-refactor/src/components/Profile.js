...
const Profile = (props) => {
  ...
  const fetchGitHubInfo = () => {
    ...
    const infoUrl = `${ GITHUB_BASE_URL }/${ props.match.params.user }${ GITHUB_TOKENS }`;
    axios.get(infoUrl)
    .then( res => {
      console.log( 'user data:', res.data );
    ...
    const reposUrl = `${ GITHUB_BASE_URL }/${ props.match.params.user }/repos${ GITHUB_TOKENS }`;
    axios.get(reposUrl)
    .then( res => {
      console.log( 'user repos:', res.data );
  ...
  // what is the equivalent of componentDidMount for hooks?
  useEffect( () => {
    fetchGitHubInfo();
  }, [props.match.params.user] );

  useEffect( () => {
    console.log('Empty array triggered this function (on initial mount)');
  }, [] ); // <-- empty array here means run only once on mount

  useEffect( () => {
    console.log('Absence of array causes this function to trigger on mount AND on any change (update of state or props)!');
  });


  const { html_url, name, public_repos, followers } = userInfo;

  return (
    <div>
      <h2>Profile of { props.match.params.user }</h2>
      ...
      <div className="repos">
        <ul>
          {
            userRepos.map( r => <li key={ r.id }>{ r.name }</li> )
          }
        </ul>
      </div>
